...
class ParquetStorageWrapper:
    ...
    ### Usage:
    ```python
    from agentChef.crawlers.crawlers_module import ParquetStorageWrapper

    # Initialize storage
    storage = ParquetStorageWrapper(data_dir="./data")
    ...
    # Load data
    df = storage.load_from_parquet("output.parquet")

    # Save YouTube/GitHub data
    storage.save_youtube_data(video_data, video_id="123", data_type="metadata")
    storage.save_github_data(repo_data, owner="user", repo="project")
    
    # Prepare data for vector embedding
    vectors = storage.prepare_for_vectors("output.parquet", text_columns=["text"], id_column="id")
    
    # Append data to existing Parquet file
    storage.append_to_parquet(data, "output.parquet")
    ```
    """
    ...
    def load_from_parquet(self, file_path: Union[str, Path]) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            self.logger.error(f"Error loading from parquet: {str(e)}")
            return None
    ...
    def append_to_parquet(self, data: Union[Dict, List, pd.DataFrame], file_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error appending to parquet: {str(e)}")
            return False

    def save_youtube_data(self, data: Union[Dict, List, pd.DataFrame], 
                         video_id: Optional[str] = None,
                         data_type: str = "metadata") -> str:
        ...
        """
        try:
            return self.storage.save_youtube_data(
                data, 
                video_id=video_id,
                data_type=data_type,
            ...
            self.logger.error(f"Error saving YouTube data: {str(e)}")
            return ""

    def save_github_data(self, data: Union[Dict, List, pd.DataFrame], 
                        owner: str, 
                        repo: str) -> str:
        ...
        """
        try:
            return self.storage.save_github_data(
                data,
                owner=owner,
                repo=repo,
                base_dir=self.data_dir
            ...
            self.logger.error(f"Error saving GitHub data: {str(e)}")
            return ""

    def prepare_for_vectors(self, 
        ...
        """
        try:
            return self.storage.prepare_for_vectors(
                file_path,
            ...
            self.logger.error(f"Error preparing vectors: {str(e)}")
            return []